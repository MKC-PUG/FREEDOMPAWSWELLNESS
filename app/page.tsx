...
export default function FreedomPawsDashboard() {
  ...
  const handleViTUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setTimeout(() => {
      setAnalysis(`✅ AI Analysis Complete!\n\n**Recommended Protocol:**\n• Allergy Shield (skin/coat issues)\n• Buddy's Gut Balance (digestive signs)\n\nFull real AI coming soon.`);
    }, 1400);
  };
      ...
      <div className="max-w-6xl mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="bg-[#1F2A44] hover:bg-[#2A3A5A] p-8 rounded-3xl border border-[#334155] transition">
            ...
            {vitImage && (
              <div className="mt-6 border border-[#F5C242]/30 rounded-3xl overflow-hidden">
                <img src={vitImage} alt="Uploaded dog" className="w-full h-auto" />
              </div>
            )}

            {analysis && <div className="mt-4 p-4 bg-black/60 rounded-2xl text-sm">{analysis}</div>}
          </div>
          ...
          <div className="bg-[#1F2A44] hover:bg-[#2A3A5A] p-8 rounded-3xl border border-[#334155] transition">
            ...
            />
            <button onClick={viewAllRecords} className="mt-4 w-full bg-green-600 hover:bg-green-500 py-3 rounded-2xl font-bold">
