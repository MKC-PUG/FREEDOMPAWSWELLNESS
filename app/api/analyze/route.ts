...
export async function POST(request: NextRequest) {
  try {
    ...
    if (filename.includes("rash") || filename.includes("skin") || filename.includes("itch") || filename.includes("red")) {
      protocol = "Allergy Shield – Skin & Coat Glow";
      finding = "Skin inflammation or allergies detected.";
    } else if (filename.includes("poop") || filename.includes("stool") || filename.includes("gut") || filename.includes("digest")) {
      protocol = "Buddy's Gut Balance & Cleanse";
      finding = "Digestive imbalance or gut issues detected.";
    } else if (filename.includes("eye") || filename.includes("vision") || filename.includes("face")) {
      protocol = "Clear Vision Defender";
      finding = "Eye or facial area concern detected.";
    } else if (filename.includes("leg") || filename.includes("joint") || filename.includes("move") || filename.includes("limp")) {
    ...
    return NextResponse.json({
      ...
      protocol: protocol,
      confidence: "78%",
      summary: "Analysis based on uploaded image."
    });
