...
flowchart TB
  subgraph OUTREACH["Outreach channels"]
    EML["Email sequences\npartners@ / shelter@"]
    SMS["SMS opt-in only\nfounding + events"]
    SOC["Social P2P\nIG · FB · TikTok · LinkedIn"]
    DIR["Direct\nclinics · shelters · VFW posts"]
    AFF["Affiliate networks\nImpact · ShareASale · CJ"]
  end

  subgraph TARGETS["300-target directory (50 x 6)"]
    INS50["Insurance 50"]
    TEL50["Telehealth 50"]
    FOOD50["Nutrition 50"]
    TOY50["Chews/toys 50"]
  ...
  subgraph ONBOARD["Onboarding gates"]
    STD["/wellness/partners standards"]
    LEG["Legal + FTC disclosure"]
    ENV["Vercel partner URLs"]
    QA["Funnel QA"]
  end

  subgraph APP["Freedom Paws app funnels"]
    VIT["ViT → concern/urgent"]
    ID["ID enroll → lost-dog"]
    WELL["/wellness cards"]
    PROT["Protocols → affiliates"]
    SHL["Shelter found→match→email"]
    VETAPP["Veteran sponsored seats"]
  end

  subgraph REV["Revenue + mission"]
    CPA["Insurance CPA"]
    REF["Telehealth referral"]
    AFFREV["Nutrition/toy affiliate"]
    B2B["Shelter B2B future"]
    GIVE["10% give-back"]
  end
